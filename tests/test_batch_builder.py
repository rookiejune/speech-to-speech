...
class CausalLMBatchBuilderTest(unittest.TestCase):
    def test_autoregression_builds_padded_global_batch(self) -> None:
        builder = CausalLMBatchBuilder(_embedding(), tokenizer=MockTokenizer())
    ...
    def test_chat_template_mapping_output_is_supported(self) -> None:
        builder = CausalLMBatchBuilder(_embedding(), tokenizer=MappingTokenizer())
...
def _embedding() -> IdSpaceEmbedding:
    space = IdSpace(
        ...
        [
            ModalityBlock(Modality.TEXT, 0, 16),
            ModalityBlock(Modality.AUDIO, 18, 5),
        ],
    )
