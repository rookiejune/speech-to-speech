...
echo '{"event":"job.launch","experiment":"train/fdu_stage_4_full_sequence_smoke","stage":"stage_4","objective":"full_codec_sequence","entry":"train"}'
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}" "${SPEECH_TO_SPEECH_PYTHON}" scripts/train.py \
  experiment=train/fdu_stage_4_full_sequence_smoke \
