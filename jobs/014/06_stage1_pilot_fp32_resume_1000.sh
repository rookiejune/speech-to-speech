...
checkpoint="${SPEECH_TO_SPEECH_STAGE_CKPT_PATH:?set SPEECH_TO_SPEECH_STAGE_CKPT_PATH}"
qwen_root="$(fdu_qwen_root)"
visible_devices="${CUDA_VISIBLE_DEVICES:-${SPEECH_TO_SPEECH_STAGE_GPUS:-1,2}}"

cd "${SPEECH_TO_SPEECH_ROOT}"
echo "{\"event\":\"job.launch\",\"entry\":\"scripts/train.py\",\"experiment\":\"train/014_stage1_pilot_fp32_resume_1000\",\"devices\":\"${visible_devices}\"}"
