...
- [codec oracle](design/codec_oracle.md)：codec screening 的模型与 prepared-code 数据边界。
...
正常训练只表达一种合法组合：所有 batch 计算 semantic CE；audio-target batch 额外计算模型对应的 acoustic objective。`semantic`、`flow_matching` 等布尔开关不用于表达正常训练路径；oracle 使用独立 objective/入口，REPA 通过显式 teacher 与数值权重加入。
...
Model 提供 semantic generation 与 acoustic sampling 原语；上层 generation service 负责任务
状态机、allowed tokens、变长裁剪、frame 对齐和 decode。同一次请求的 token 与
`AudioOutput(features, waveform, sample_rate)` 必须来自同一次生成结果；采样率由生成所用
codec 提供，不由日志或调用方猜测。
