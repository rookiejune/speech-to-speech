...
`Trainer.fit` 到达 `max_steps=2` 之后的 greedy generation：`generate_responses()` 对真实
`SpeechToSpeechRVQModel` 抛出
...
1. 为 generation capability 增加最小回归测试：
   ...
   - 断言 `generate_responses()` 在 codec 有 acoustic codebooks 时调用 audio feature generation，
     不因 Protocol false negative 报错。
   - 保留现有 token-only `TokenModel` 对 acoustic-codebook codec 报错的负例。
2. 修复能力检查：
   - 不再依赖对整个 `TokenGenerator` 的 runtime Protocol `isinstance` 判断可选能力。
   - 改为显式检查 callable `generate_audio_features`，并在调用前把模型收窄到
     `AcousticFeatureGenerator` 类型；缺失时继续暴露原来的清晰 `TypeError`。
   - 不改变 `SpeechToSpeechRVQModel.generate_audio_features()`、RVQ sampling 或 decode 逻辑。
3. 本地验证：
   - 运行聚焦的 generation 单测。
...
4. 复旦复测：
   ...
   - 在 `145` 上使用本地 `/tmp` Qwen snapshot 和 prepared data root，运行
...
SPEECH_TO_SPEECH_P0_QWEN_ROOT=/tmp/s2s-011-hf/hub/models--Qwen--Qwen3-0.6B/snapshots/c1899de289a04d12100db370d81485cdf75e47ca \
