...
1. [简单介绍](plan/overview.md)：说明 V1 失败原因、外部 generator plugin 边界和四阶段总体思路。
...
generator plugin 的 conditioner、acoustic generator 预训练和 artifact 导出由仓库外的
`semantic-acoustic-generator` 完成。本仓库在 composition 时通过
`model.acoustic.init_artifact` 显式消费 frame-aligned generator artifact，不把 generator plugin 训练或其他 codec
