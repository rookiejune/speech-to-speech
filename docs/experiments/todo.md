...
- 测试现状：66 个纯本地测试覆盖 audio tokenizer、P0 数据/ID 契约、
...
- 重新验收 `schedules/005-acoustic-oracle-codec-screening.md` 的 LongCat acoustic flow 路径：
  当前实现已改为完整复用 `SpeechToSpeechFlowModel`，旧 121 smoke 只覆盖独立 flow model，
  不能作为新路径结论。
