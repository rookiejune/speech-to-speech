...
- `model.train_mode`：控制参数训练范围；默认 `default` 按 backbone/token/acoustic 配置分别控制，`acoustic_only` 用于声学消融，只训练 audio embedding、DiT acoustic decoder 和 acoustic condition projection。
