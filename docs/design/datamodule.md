...
- `types.Task` / `types.Language`：任务与语言枚举。`Task` 是 source/target modality 与 paired 语义的唯一事实来源。
- `task.TaskFactory` / `task.TaskBase`：按任务把 `SpeechPair` 组装成 `Sample`，包括 chat template、boa/eoa 包装、labels 与 acoustic positions 的生成。
- `Collator`：按权重策略采样任务并 collate。
- `DataModule`：Lightning 数据入口，通过 `wmt19_tts_codec(config.codec)` 选择当前工程使用的具体 codec view；持有可更新的 `Collator`，`set_strategy()` 由外部 callback 控制任务权重。
...
- `Speech.bpe_spans` 在构造时校验 spans 与 semantic frame 严格双射；不满足直接报错，不做静默修复。
