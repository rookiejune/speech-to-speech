...
- `text_tokenizer`：backbone 对应的 HF tokenizer。
...
`types.py` 定义消费方依赖的结构化协议：`Codec`、`AudioTokenizer`、`TextTokenizer`、`Backbone`。fake 实现只需满足协议即可支撑 contract test（`dummy.py` 暴露 `NativeAudioTokenizer` 作为 fake tokenizer）。
