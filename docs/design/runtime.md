...
`runtime/types.py` 定义 model/generation 依赖的 `Codec`、`AudioTokenizer`、`TextTokenizer` 与
`Backbone` Protocol。`datamodule.DataRuntime` 是更窄的只读协议，只公开 parser、sample builder
和 batch padding 所需资源。
...
- `_CodecContract` 隔离具体 LongCat/UniCodec 类型，模型只依赖本地 Protocol。
