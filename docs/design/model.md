...
- `protocol.FlowMatching`：loss 依赖的训练能力；`protocol.SemanticGeneration` /
  `protocol.FlowGeneration`：generation service 依赖的推理能力；`protocol.FlowModel`
  组合 Lightning module 所需的训练与推理契约。
...
- generation 已知 allowed token IDs 时，内部只计算对应 modality 的 output head，并直接在
  该 ID 子集对应的 output weight rows，并只对最后一个 token 计算 logits；这条私有优化
  不改变公开 `forward()` 的 global logits 契约。
...
- condition 接口 `target_frame_condition()` / `target_frame_label_condition()` 统一消费 token
  ...
  `FlowMatching` 训练协议，后者是具体模型提供的 teacher-forced/oracle 能力。
